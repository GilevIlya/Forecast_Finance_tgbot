...
from app.keyboards import build_city_keyboard, stop_operation, keyboard_of_abil
from app.services.general_serv import CurrencyAndWeatherHandlerMainClass

from app.database import validation, registration, daily_count, save_city_and_coords
...
async def process_help_command(message: Message):
    await message.answer(
        ...
        "💱 Курсы валют относительно UAH/🇺🇦 Украинская гривна:\n"
        "------------------------------------\n"
        "• AZN/🇦🇿 Азербайдж. манат: 24.6397\n"
        "• CZK/🇨🇿 Чешская крона: 1.9902\n"
        "• EUR/🇪🇺 Евро: 48.4088\n"
        "• MDL/🇲🇩 Молдавский лей: 2.4624\n"
        "• PLN/🇵🇱 Польский злотый: 11.3727\n"
        "• RON/🇷🇴 Румынский лей: 9.5182\n"
        "• USD/🇺🇸 Доллар США: 41.8924\n"
        "------------------------------------\n"
        "🛈 Данные актуальны на 2025-11-02.\n\n"
...
async def process_change_city(message: Message, state: FSMContext):
    ...
    if current_city is None:
        ...
        await message.answer(f'🏢 Ваш нынешний город: {city}\n'
                             '🚀 Введите новый город, и я телепортируюсь туда прогнозом погоды!',
                             reply_markup=stop_operation)
        await state.set_state(Register.city)
...
async def register_city_FSMContext(message: Message, state: FSMContext):
    city = message.text
    if city[0] == '/':
        await state.clear()
    if len(city) > 50:
...
async def create_api_params(city):
...
async def get_weather_forecast(url, city):
    params = await create_api_params(city)
    async with aiohttp.ClientSession() as session:
        async with session.get(url, params=params) as resp:
            data = await resp.json()
            if resp.status == 200:
...
class WeatherCommandsHandler(CurrencyAndWeatherHandlerMainClass):
    configs = {
        '/weather': {
            ...
            'emoji': '🌦',
            'desc': 'нынешнюю погоду'
        },
        '/weather_week': {
            'validkey': 'weatherweek_daily_count',
            'limit': 3,
            'get_weather': get_weatherweek,
            'emoji': '☀️',
            'desc': 'погоду на пять дней'
        }
    }
    ...
    async def main_process(self, message: Message, state: FSMContext):
        ...
        key = self._define_key(message.text)
        config = self.configs[key]
        user_attempts = await validation(user_id, config['validkey'])
        if await self._is_limit_reached(config['limit'], user_attempts):
            await message.answer(f"🌤 Кажется, сегодня вы уже всё выяснили про {config['desc']}😊\n"
                                "Лимит запросов исчерпан — приходите завтра, узнаем, что готовит небо!")
            return
        location = await validation(user_id, 'cityandcoords')
        if location is None:
            return await self._ask_for_city(message, state)
        await message.answer('⏳ Подождите...')
        weather_data = await self._get_weather_data(user_id, config)
        await (message.answer("\n".join(weather_data)) if key == '/weather_week'
               else message.answer(weather_data))
        await self._count_message(config['limit'], user_attempts, message, weather_config=config)
        await self._update_counter(user_id, config['validkey'])

    async def _ask_for_city(self, message, state):
        await message.answer("❌ Упс, я пока не знаю, где ты живёшь.\n"
                                    "Введи свой город 🌆, и в следующий раз я смогу подсказать тебе погоду одним движением!",
                                  reply_markup=stop_operation)
        await state.set_state(Register.city)

    async def _get_weather_data(self, user_id, config):
        city = await validation(user_id, 'cityandcoords')
        return await config['get_weather'](city)

    def _define_key(self, text):
        if text in ('🌦 Погода', '/weather'):
            return '/weather'
        elif text in ('☀️Погода на 5 дней', '/weather_week'):
            return '/weather_week'
