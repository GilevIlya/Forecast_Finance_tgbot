...
async def weather(message: Message, state:FSMContext):
    weather_operations = WeatherHandler(message, state)
    await weather_operations.main_process()
...
async def change_city(message:Message, state:FSMContext):
    ...
    if current_city is None:
        await message.answer("❌ Упс, я пока не знаю, где ты живёшь.\n"  
                            "Введи свой город 🌆, и в следующий раз я смогу подсказать тебе погоду одним движением!", reply_markup=stop_operation)
...
class WeatherHandler:
    ...
    def __init__(self, message, state):
        ...
        self.key = self._define_key(message.text)
        self.config = self.configs[self.key]

    async def main_process(self):
        if await self._is_limit_reached():
            await self.message.answer(f"🌤 Кажется, сегодня вы уже всё выяснили про {self.config['desc']}😊\n"
                                "Лимит запросов исчерпан — приходите завтра, узнаем, что готовит небо!")
            return
        location = await validation(self.user_id, 'cityandcoords')
        if location is None:
            return await self._ask_for_city()
        await self.message.answer('⏳ Подождите...')
        weather_data = await self._get_weather_data()
        await (self.message.answer("\n".join(weather_data)) if self.key == '/weather_week'
               else self.message.answer(weather_data))
        await self._counter_message()
        await self._udpate_counter()


    async def _is_limit_reached(self):
        self.user_attempts = await validation(self.user_id, self.config['validkey'])
        return self.config['limit'] <= self.user_attempts if self.user_attempts else 0

    async def _ask_for_city(self):
        await self.message.answer("❌ Упс, я пока не знаю, где ты живёшь.\n"
    ...
    async def _get_weather_data(self):
        city = await validation(self.user_id, 'cityandcoords')
        return await self.config['get_weather'](city)
    ...
    async def _udpate_counter(self):
        await daily_count(self.user_id, self.config['validkey'])
